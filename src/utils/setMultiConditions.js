...
export const setLevelColor = (level) => {
    if (level === 'Ưu tiên') {
        return 'inline-block w-fit align-text-top level priority';
    } else if (level === 'Khẩn cấp') {
        return 'inline-block w-fit align-text-top level emergency';
    } else {
        return 'inline-block w-fit align-text-top level normal';
    }
};
